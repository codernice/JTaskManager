        ...
        public void Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                foreach (var item in StatusJobList)
                {
                    try
                    {
                        var exists = QuartzHelper.CurrentTaskList.FirstOrDefault(q => q.TaskID == item.TaskID);
                        if (item.Status == Core.TaskStatus.RUN)
                        {
                            QuartzHelper.ResumeJob(item.TaskID.ToString());
                            if (exists != null)
                            {
                                exists.Status = Core.TaskStatus.RUN;
                            }
                        }
                        else
                        {
                            ...
                            if (exists != null)
                            {
                                exists.Status = Core.TaskStatus.STOP;
                            }
                        }
                    }
                    catch (Exception e)
                    {
                        Logger.Default.Error("自动暂停任务失败，错误：" + e.Message + e.StackTrace);
                    }
                }
                ...
                foreach (var item in UpdateJobList)
                {
                    ...
                    {
                        Logger.Default.Error("自动修改任务失败，错误：" + e.Message + e.StackTrace);
                    }
                }
                ...
                foreach (var taskModel in AddJobList)
                {
                    ...
                    {
                        Logger.Default.Error("自动增加任务失败，错误：" + e.Message + e.StackTrace);
                    }
                }
                ...
                foreach (var taskModel in DeleteJobList)
                {
                    ...
                    {
                        Logger.Default.Error("自动删除任务失败，错误：" + e.Message + e.StackTrace);
                    }
                }
                if (UpdateJobList.Count > 0 || AddJobList.Count > 0 || DeleteJobList.Count > 0)
                {
                    Logger.Default.Error("Job修改任务执行完成后,系统当前的所有任务信息:" + JsonHelper.ToJson(QuartzHelper.CurrentTaskList));
                }
                else
                {
                    Logger.Default.Error("当前没有修改的任务");
                }
            }
            catch (Exception ex)
            {
