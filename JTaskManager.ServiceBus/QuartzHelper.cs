...
namespace JTaskManager.ServiceBus
{
    ...
    public class QuartzHelper
    {
        ...
        private static IList<Tasks> _currentTaskList = null;

        /// <summary>
        /// 
        /// </summary>
        /// <returns></returns>
        public static IList<Tasks> CurrentTaskList
        {
            set { }
            get
            {
                ...
                {
                    _currentTaskList = TasksOperate.GetAllTaskList();
                }
            ...
            try
            {
                lock (obj)
                {
                    if (scheduler == null)
                    {
                        ...
                        Logger.Default.Info("任务调度初始化成功！");
                    }
                }
            }
            catch (Exception ex)
            {
                Logger.Default.Error("任务调度初始化失败！", ex);
            }
        }
        ...
        public static void StartScheduler()
        {
            try
            {
                if (!scheduler.IsStarted)
                {
                    ...
                    scheduler.Start();
                    ///获取所有执行中的任务
                    ITasksService tasks = new TasksService();
                    Task<ApiResult<List<Tasks>>> result = tasks.GetListAsync();
                    List<Tasks> listTask = result.Result.data;
                    _currentTaskList = listTask;
                    if (listTask != null && listTask.Count > 0)
                    {
                        foreach (Tasks taskUtil in listTask)
                        {
                            try
                            {
                            ...
                            {
                                Logger.Default.Error(string.Format("任务“{0}”启动失败！", taskUtil.TaskName), e);
                            }
                        }
                    }
                    Logger.Default.Info("任务调度启动成功！");
                }
            }
            catch (Exception ex)
            {
                Logger.Default.Error("任务调度启动失败！", ex);
            }
        }
        ...
        public static void InitRemoteScheduler()
        {
            ...
            {
                Logger.Default.Error("初始化远程任务管理器失败" + ex.StackTrace);
            }
        }
        ...
        public static void DeleteJob(string JobKey)
        {
            try
            {
                ...
                if (scheduler.CheckExists(jk))
                {
                    ...
                    Logger.Default.Info(string.Format("任务“{0}”已经删除", _currentTaskList.FirstOrDefault(q => q.TaskID.ToString() == JobKey).TaskName));
                }
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        public static void ScheduleJob(Tasks task, bool isDeleteOldTask = false)
        {
            if (isDeleteOldTask)
            {
                //先删除现有已存在任务
            ...
            {
                var assemblyName = task.AssemblyName;
                #if DEBUG
                if (!string.IsNullOrEmpty(task.AssemblyNameDebug))
                {
                    Logger.Default.Info("#########程序集取Debug路径#########");
                    assemblyName = task.AssemblyNameDebug;
                }
                #endif
                IJobDetail job = new JobDetailImpl(task.TaskID.ToString(), GetClassInfo(assemblyName, task.ClassName));
                //添加任务执行参数
                ...
                if (task.Status == JTaskManager.Core.TaskStatus.STOP)
                {
                ...
                {
                    Logger.Default.Info(string.Format("任务“{0}”启动成功,未来5次运行时间如下:", task.TaskName));
                    List<DateTime> list = GetNextFireTime(task.CronExpressionString, 5);
                    foreach (var time in list)
                    {
                        Logger.Default.Info(time.ToString());
                    }
                }
            }
            else
        ...
        public static void PauseJob(string JobKey)
        {
            try
            {
                ...
                if (scheduler.CheckExists(jk))
                {
                    //任务已经存在则暂停任务
                    scheduler.PauseJob(jk);
                    Logger.Default.Info(string.Format("任务“{0}”已经暂停", _currentTaskList.FirstOrDefault(q=>q.TaskID.ToString() == JobKey).TaskName));
                }
            }
            catch (Exception ex)
            {
        ...
        public static void ResumeJob(string JobKey)
        {
            try
            {
                ...
                if (scheduler.CheckExists(jk))
                {
                    ...
                    Logger.Default.Info(string.Format("任务“{0}”恢复运行", _currentTaskList.FirstOrDefault(q => q.TaskID.ToString() == JobKey).TaskName));
                }
            }
            catch (Exception ex)
            {
                Logger.Default.Error("恢复任务失败！", ex);
            }
        }
        ...
        public static void StopSchedule()
        {
            try
            {
                ...
                if (!scheduler.IsShutdown)
                {
                    ...
                    Logger.Default.Info("任务调度停止！");
                }
            }
            catch (Exception ex)
            {
                Logger.Default.Error("任务调度停止失败！", ex);
            }
        }
        ...
        /// <returns></returns>
        public static Tasks GetTaskDetail(IJobExecutionContext context)
        {
